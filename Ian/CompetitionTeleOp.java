...
public class CompetitionTeleOp extends OpMode{
    ...
    public void checkOperatorControls(){
        ...
        // Intake controls
        //the header for the intake controls.
        ...
        if(Math.abs(ly) > thresh) { // Strong enough
            ...
            else if(ly > 0 && upDpad ){// trubo going down
                robot.intake.intakeArmRaiseLower(1/2, true, false);
            }
            else if(ly<0) { // down
                robot.intake.intakeArmRaiseLower(Math.abs(ly),false,true);
            }
            else if(ly < 0 && downDpad){// trubo for going down if we need it
                robot.intake.intakeArmRaiseLower(1/2,false,true);
            }
        }
        else { // Not strong enough
            robot.intake.intakeArmRaiseLower(0,false,false);
        }
        // extends the arm forward and back words
