...
public class CompetitionAutoOp extends LinearOpMode{
    ...
    public void allTheWays(double rotatePower, double linearPower) throws InterruptedException {
        ...
        encoderDrive(0.2, robot.distanceToAvoidMineral, 30, true, robot.curHeading, true, true, 0);
        startTimer();
        while (runtime.seconds() < 2){
            //idle time
        }
        ...
        // going to the depot
        encoderDrive(linearPower, robot.distanceToDepot, 30, true, robot.curHeading, true, true, 0);
        startTimer();
        while (runtime.seconds() < 2){
            // idle time
        }
        ...
        if (linearPower > 0){
            encoderDrive(-0.2,robot.distanceFromDepotToCrater, 30, true, robot.curHeading, true, true, 0);
        }else{
            encoderDrive(0.2, robot.distanceFromDepotToCrater, 30, true, robot.curHeading, true, true, 0);
        }
    ...
    public class DriveThread extends Thread {
        ...
        public DriveThread(double power, double distance, boolean rotation) {
            this.inchesOrDegress = distance;
            this.power = power;
            this.rotation = 1;
        }
        ...
        public void driveInInches (double power,double inches,boolean rotation) {
            boolean busy = true;
            double rev = inches / robot.wheelCircumference;
            int counts = (int)(rev * robot.encCountsPerRev);
            int sign = 1;
            ...
            if (rotation) {
                for (int i = 0; i < robot.motors.length; i++) {
                    ...
                    int currentPosition = robot.motors[i].getCurrentPosition();
                    robot.motors[i].setMode(DcMotor.RunMode.RUN_TO_POSITION);
                    robot.motors[i].setTargetPosition(currentPosition + sign * counts);
                    robot.motors[i].setPower(Math.abs(power)*sign);
                }
            } else {
                for (int i = 0; i < robot.motors.length; i++) {
                    int even = i%2;
                    ...
                    robot.motors[i].setMode(DcMotor.RunMode.RUN_TO_POSITION);
                    robot.motors[i].setTargetPosition(currentPosition + sign * counts);
                    robot.motors[i].setPower(sign*Math.abs(power));
                }
            }

            while (busy) {
                for (int i = 0; i < robot.motors.length; i++) {
                    busy = busy && robot.motors[i].isBusy();
                }
            }
        ...
        public void rotateInDegrees(double power,double degrees){
            driveInInches(power,degrees/360*robot.robotCircumference,true);
        }
        ...
        public void gyroTurn(double speed, double angle, double coefficient) throws InterruptedException { // INTERRUPTED EXCEPTION WAS NOT HERE BEFORE, BRANDON PUT IT IN THERE TO SOLVE SLEEP LINE ERROR
            ...
            while (opModeIsActive() && !onHeading(speed, angle, coefficient)) {
                ...
                sleep(1);                                                                           // LINE OF CODE I'M REFERRING TO
